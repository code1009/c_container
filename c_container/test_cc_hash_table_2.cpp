...
static bool data_memory_pool_initialize()
{
	...
	if (rv == false)
	{
		std::cout << "[error] data storage allocator initialize failed" << std::endl;
		cc_debug_assert(0);
		return false;
	}
	return true;
...
static data_t* data_memory_pool_alloc()
{
	...
	{
		std::cout << "[error] data_memory_pool_alloc() failed" << std::endl;
		//cc_debug_assert(0);
	}
	return data_pointer;
}
...
static void data_memory_pool_free(data_t* data)
{
	...
	if (rv == false)
	{
		std::cout << "[error] data_memory_pool_free() failed" << std::endl;
		cc_debug_assert(0);
	}
}
...
static void add(void)
{
	...
	for (i = 0; i < count; i++)
	{
		...
		{
			std::cout << "[error] data storage allocate failed:" << index_string(i) << std::endl;
			break;
		}
		...
		if (false == rv)
		{
			std::cout << "[error] add failed:" << index_string(i) << std::endl;
